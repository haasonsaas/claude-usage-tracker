...
describe("Analyzer", () => {
	...
	describe("analyzeModelEfficiency", () => {
		it("should calculate efficiency metrics correctly", () => {
			...
			const sonnetEfficiency = efficiency.find(
				(e) => e.model === "claude-3.5-sonnet-20241022",
			);
			const opusEfficiency = efficiency.find(
				(e) => e.model === "claude-opus-4-20250514",
			);
			...
			expect(Number.isFinite(sonnetEfficiency!.avgTokensPerConversation)).toBe(
				true,
			);
