...
export class ModelAdvisor {
  ...
  private estimateTokenUsage(taskType: TaskClassification['taskType']): { input: number; output: number } {
    const estimates = {
  ...
  private getReasoningForTask(taskType: TaskClassification['taskType'], model: 'sonnet' | 'opus'): string {
    const reasons = {
      ...
      debugging: {
        sonnet: 'Sonnet 4 can handle most debugging tasks effectively with significant cost savings.',
        opus: 'Opus 4 recommended for complex debugging - better at understanding intricate code relationships.'
      },
      code_review: {
        sonnet: 'Sonnet 4 provides thorough code reviews with excellent cost efficiency.',
        opus: 'Opus 4 for critical code reviews where you need the deepest analysis.'
      },
      documentation: {
        sonnet: 'Sonnet 4 is perfect for documentation - clear writing with major cost savings.',
        opus: 'Opus 4 overkill for most documentation tasks.'
      },
      architecture: {
        sonnet: 'Sonnet 4 can handle many architecture discussions cost-effectively.',
      ...
      simple_query: {
        sonnet: 'Sonnet 4 perfect for straightforward questions - massive cost savings.',
        opus: 'Opus 4 wasteful for simple queries.'
      },
      refactoring: {
        sonnet: 'Sonnet 4 excellent for refactoring with great cost efficiency.',
  ...
  formatRecommendation(classification: TaskClassification, recommendation: ModelRecommendation): string {
    ...
    // Task classification
    output += chalk.cyan('Task Type: ') + chalk.white(classification.taskType.replace('_', ' ')) + 
