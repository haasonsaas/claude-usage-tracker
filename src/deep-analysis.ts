...
export class DeepAnalyzer {
	generateDataQualityReport(entries: UsageEntry[]): DataQualityReport {
		...
		const spanDays = Math.ceil(
			(latestTime - earliestTime) / (1000 * 60 * 60 * 24),
		);
		...
		const top10PercentCost = sortedCosts
			.slice(top10PercentIndex)
	...
	generateUsageHeatmap(entries: UsageEntry[]): UsageHeatmap {
		...
		const hourlyHeatmap = Array.from(heatmapData.entries()).map(
			([key, data]) => {
		...
		const monthlyTrends = Array.from(monthlyData.entries()).map(
			([month, data]) => {
				const topModel =
					Object.entries(data.modelCounts).sort(
						([, a], [, b]) => b - a,
					)[0]?.[0] || "unknown";
		...
		const efficiencies = entries.map(
			(e) => e.total_tokens / Math.max(calculateCost(e), 0.001),
		);
		const ranges = [
		...
		const efficiencyDistribution = ranges.map((range) => {
			const count = efficiencies.filter(
				(eff) => eff >= range.min && eff < range.max,
			).length;
			return {
				range: range.range,
				count,
	...
	analyzeConversationFlow(entries: UsageEntry[]): ConversationFlowAnalysis {
		...
		const conversationJourneys = Array.from(conversations.entries()).map(
			([id, convEntries]) => {
				...
				const avgResponseTime =
					sorted.length > 1 ? totalGaps / (sorted.length - 1) / 1000 : 0;
				...
				} else if (avgResponseTime > 30 * 60 * 1000) {
		...
		const flowPatterns = Array.from(patterns.entries()).map(
			([pattern, data]) => ({
				pattern,
	...
	analyzeCostDrivers(entries: UsageEntry[]): CostDriverAnalysis {
		...
		const sonnetCost = sonnetEntries.reduce(
		...
		opusConversations.forEach((conv) => {
			const avgTokens =
				conv.reduce((sum, e) => sum + e.total_tokens, 0) / conv.length;
		...
		return {
			...
			modelUsagePattern: {
				opusUsage: {
					...
					appropriateUsage:
						(appropriateOpusUsage / opusConversations.size) * 100,
	...
	private generateRecommendations(
		...
		const sprintConversations = conversationFlow.conversationJourneys.filter(
			(j) => j.conversationType === "sprint",
		);
