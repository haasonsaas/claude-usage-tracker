...
import { getCurrentWeekUsage, calculateCost } from './analyzer.js';
...
export class UsageWatcher {
  ...
  async startWatching(callback: (stats: LiveStats, recentConversations: ConversationEvent[]) => void): Promise<void> {
    ...
    for (const dataPath of CLAUDE_DATA_PATHS) {
      try {
        ...
        if (stats.isDirectory()) {
          ...
          const watcher = watch(dataPath, { recursive: true }, async (_eventType, filename) => {
  ...
  private async handleFileChange(): Promise<void> {
    try {
      ...
      const newEntries = entries.filter(entry => 
        new Date(entry.timestamp) > this.lastProcessedTime
      );
  ...
  private async updateStats(): Promise<void> {
    try {
      ...
      this.lastStats = {
        ...
        averageCostPerConversation: todayConversations > 0 ? todayCost / todayConversations : 0
      };
  ...
  formatLiveDisplay(stats: LiveStats, recentConversations: ConversationEvent[]): string {
    ...
    output += chalk.blue.bold('🔴 LIVE USAGE MONITOR') + chalk.gray(` (${now.toLocaleTimeString()})\n`);
    ...
    // Burn rate indicator
    const burnRateColor = stats.burnRate > 20 ? chalk.red : 
                         stats.burnRate > 0 ? chalk.yellow : chalk.green;
    const burnRateIcon = stats.burnRate > 20 ? '🔥' : 
                        stats.burnRate > 0 ? '📈' : '📉';
    ...
    if (stats.lastConversationModel !== 'N/A') {
      const modelName = stats.lastConversationModel.includes('opus') ? 'Opus 4' : 'Sonnet 4';
      const modelColor = stats.lastConversationModel.includes('opus') ? chalk.magenta : chalk.blue;
    ...
    if (recentConversations.length > 0) {
      ...
      for (const conv of recent) {
        ...
        const efficiencyIcon = conv.efficiency === 'high' ? '⭐⭐⭐' : 
    ...
    if (stats.burnRate > 50) {
      ...
      output += chalk.red('Consider switching to Sonnet 4 for simpler tasks\n\n');
    } else if (stats.averageCostPerConversation > 50) {
      output += chalk.yellow.bold('💡 EFFICIENCY TIP\n');
      output += chalk.yellow('Average conversation cost is high - consider breaking down complex tasks\n\n');
    }
