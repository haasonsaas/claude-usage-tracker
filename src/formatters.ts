...
import type { DailyUsage, RateLimitInfo, WeeklyUsage } from "./types.js";
...
export function formatWeeklySummary(weeklyUsage: WeeklyUsage): string {
	...
	table.push(
		...
		[
			...
			`${formatTokenCount(
				weeklyUsage.cacheCreationTokens + weeklyUsage.cacheReadTokens,
			)}`,
		],
...
export function formatRateLimitStatus(rateLimitInfo: RateLimitInfo): string {
	...
	const sonnet4Usage = `${formatHours(
		rateLimitInfo.currentUsage.estimatedHours.sonnet4.min,
	)} hrs`;
	const sonnet4Limit = `${rateLimitInfo.weeklyLimits.sonnet4.min}-${rateLimitInfo.weeklyLimits.sonnet4.max} hrs`;
	const sonnet4Percent = `${formatPercentage(
		rateLimitInfo.percentUsed.sonnet4.min,
	)}`;
	const sonnet4Status =
		rateLimitInfo.percentUsed.sonnet4.min > 80
			? chalk.red("⚠️  High")
	...
	const opus4Usage = `${formatHours(
		rateLimitInfo.currentUsage.estimatedHours.opus4.min,
	)} hrs`;
	const opus4Limit = `${rateLimitInfo.weeklyLimits.opus4.min}-${rateLimitInfo.weeklyLimits.opus4.max} hrs`;
	const opus4Percent = `${formatPercentage(
		rateLimitInfo.percentUsed.opus4.min,
	)}`;
	const opus4Status =
		rateLimitInfo.percentUsed.opus4.min > 80
			? chalk.red("⚠️  High")
...
export function formatModelEfficiency(
	...
	const sortedModels = modelEfficiency.sort(
...
`;
	...
	if (insights.costSavingsOpportunity.potentialSavings > 100) {
		...
		output += chalk.green(
			`Potential monthly savings: $${insights.costSavingsOpportunity.potentialSavings.toFixed(
				0,
			)}
`,
		);
